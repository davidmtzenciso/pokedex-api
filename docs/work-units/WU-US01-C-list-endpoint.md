...
| 3 | **`CatalogExceptionHandler`, one advice per context** | Spring supports several advice classes, and one per context is now the rule rather than a workaround: a single global advice would import every context's exceptions and make `shared` depend on all of them (`BC3`) |
