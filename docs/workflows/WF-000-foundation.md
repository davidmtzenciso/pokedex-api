...
| IA10 | PokeAPI is unauthenticated but **fair-use rate-limited**; a naive `from=1,to=1025` batch sync issues ~3,000 requests | Bulk sync can run unbounded | Upstream 429s, or a self-inflicted outage during the live demo | [concurrency](../handbook/concurrency.md), R2 | Bounded `Semaphore(16)` + backoff; `PokeApiRateLimitComponentTest` |
